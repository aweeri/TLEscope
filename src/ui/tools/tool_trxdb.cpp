...
// copy a JSON string field into a fixed buffer
static void get_str_field(const nlohmann::json &obj, const char *key, char *dst, size_t size)
{
    dst[0] = '\0';
    auto it = obj.find(key);
    if (it != obj.end() && it->is_string())
        snprintf(dst, size, "%s", it->get_ref<const std::string &>().c_str());
}

// format a JSON number-or-string field into a fixed buffer
static void get_num_or_str_field(const nlohmann::json &obj, const char *key, char *dst, size_t size)
{
    dst[0] = '\0';
    auto it = obj.find(key);
    if (it == obj.end())
...
static void parse_object_json(const char *json, TrxdbObjectInfo *out)
{
    ...
    auto desc = root.find("description");
    if (desc != root.end() && desc->is_string())
        strip_html(desc->get_ref<const std::string &>().c_str(), out->description, sizeof(out->description));
}
...
static int parse_transponders_json(const char *json, TrxdbTransponder *out, int max)
{
    ...
    if (found != root.end() && found->is_array())
    {
        ...
        for (int i = 0; i < n; i++)
        {
            const nlohmann::json &t = found->at(i);
            ...
            auto enc = t.find("encrypted");
            tp->encrypted = (enc != t.end() && enc->is_boolean()) ? enc->get<bool>() : false;
